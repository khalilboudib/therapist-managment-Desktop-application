...
import orthophoniste.trouble;
import orthophoniste.type_categorie;
...
public class chart {
    ...
    @FXML
    void initialize(){
        for (type_categorie type :type_categorie.values()) {
            int n=0;
            for (Patient pat :app.patients){
                for (BO bo : pat.dossier.BOs){
                    for (trouble tr :bo.diagnostic.troubles){
                        if(tr.categorie.compareTo(type)==0){
                            n++;
                        }
                    }
                }
            }
            double percentage = ((double) n / app.patients.size()) * 100;
            pieChart.getData().add(new PieChart.Data(String.valueOf(type), percentage));
        }
    }
